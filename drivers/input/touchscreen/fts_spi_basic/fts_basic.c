...
/*
 * Minimal SPI touchscreen driver for ST FTS V521 style controllers.
 */
...
#define FTS_BASIC_DRV_NAME                  "fts-basic"
...
static int fts_basic_spi_write_cmd(struct fts_basic_ts *ts, const u8 *cmd,
   size_t len)
{
return spi_write(ts->spi, cmd, len);
}

static int fts_basic_spi_read_fifo(struct fts_basic_ts *ts, u8 *out,
   unsigned int events)
{
...
static int fts_basic_start_scan(struct fts_basic_ts *ts)
{
u8 cmd[3] = {
FTS_CMD_SCAN_MODE,
...
return fts_basic_spi_write_cmd(ts, cmd, sizeof(cmd));
}

static int fts_basic_stop_scan(struct fts_basic_ts *ts)
{
u8 cmd[3] = {
FTS_CMD_SCAN_MODE,
FTS_SCAN_MODE_ACTIVE,
...
static void fts_basic_release_all_slots(struct fts_basic_ts *ts)
{
unsigned int slot;
...
static irqreturn_t fts_basic_irq_thread(int irq, void *data)
{
struct fts_basic_ts *ts = data;
...
error = fts_basic_spi_read_fifo(ts, events, 1);
if (error)
goto out;
...
if (total_events > 1) {
error = fts_basic_spi_read_fifo(ts,
       events + FTS_FIFO_EVENT_SIZE,
       total_events - 1);
if (error)
goto out;
}
...
fts_basic_report_down_event(ts, event);
break;
case FTS_EVT_ID_LEAVE_POINT:
fts_basic_report_up_event(ts, event);
break;
...
static int fts_basic_power_on(struct fts_basic_ts *ts)
{
int error;
...
static void fts_basic_power_off(struct fts_basic_ts *ts)
{
if (ts->avdd)
...
static void fts_basic_hw_reset(struct fts_basic_ts *ts)
{
if (!ts->reset_gpio) {
msleep(30);
return;
...
ts->input->name = "ST FTS Touchscreen (basic)";
...
static int fts_basic_probe(struct spi_device *spi)
{
struct fts_basic_ts *ts;
...
error = fts_basic_power_on(ts);
if (error)
return error;

fts_basic_hw_reset(ts);

error = fts_basic_input_init(ts);
if (error)
goto err_power_off;
...
error = devm_request_threaded_irq(ts->dev, spi->irq, NULL,
  fts_basic_irq_thread, irq_flags,
  FTS_BASIC_DRV_NAME, ts);
if (error)
goto err_power_off;

error = fts_basic_start_scan(ts);
if (error)
goto err_power_off;
...
err_power_off:
fts_basic_power_off(ts);
return error;
}

static void fts_basic_remove(struct spi_device *spi)
{
struct fts_basic_ts *ts = spi_get_drvdata(spi);

mutex_lock(&ts->lock);
ts->suspended = true;
fts_basic_stop_scan(ts);
fts_basic_release_all_slots(ts);
input_mt_sync_frame(ts->input);
input_sync(ts->input);
mutex_unlock(&ts->lock);
...
static int __maybe_unused fts_basic_suspend(struct device *dev)
{
...
static int __maybe_unused fts_basic_resume(struct device *dev)
{
struct fts_basic_ts *ts = dev_get_drvdata(dev);
...
static const struct dev_pm_ops fts_basic_pm_ops = {
SET_SYSTEM_SLEEP_PM_OPS(fts_basic_suspend, fts_basic_resume)
};

static const struct of_device_id fts_basic_of_match[] = {
{ .compatible = "st,fts-v521-spi-basic" },
{ }
};
MODULE_DEVICE_TABLE(of, fts_basic_of_match);

static struct spi_driver fts_basic_spi_driver = {
.driver = {
.name = FTS_BASIC_DRV_NAME,
.of_match_table = fts_basic_of_match,
.pm = &fts_basic_pm_ops,
},
.probe = fts_basic_probe,
.remove = fts_basic_remove,
};
module_spi_driver(fts_basic_spi_driver);

MODULE_DESCRIPTION("Basic ST FTS V521 SPI touchscreen driver");
MODULE_LICENSE("GPL");
