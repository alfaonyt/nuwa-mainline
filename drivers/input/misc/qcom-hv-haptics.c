				...
				struct haptics_effect *effect)
{
	...
	if (!effect->fifo->preload) {
		dev_err(chip->dev, "effect doesn't support preload\n");
		return -EINVAL;
	}
...
static int haptics_probe(struct platform_device *pdev)
{
	...
	// init delay_timer
	hrtimer_setup(&(g_mhaptic.delay_off_timer), haptics_disable_fifo_delay_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	g_mhaptic.delay_ms = 0;
	...
	if ((chip->effects_count != 0) || (chip->primitives_count != 0)) {
		...
		set_bit(FF_DAMPER, input_dev->ffbit);
	}
